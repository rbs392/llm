...
from const import END_KEY, PROMPT_FOR_GENERATION_FORMAT, RESPONSE_KEY
...
model, tokenizer = load_model_tokenizer_for_generate("./custom_model/v2")
end = timeit.default_timer()
print(f"Model loaded: time taken: {end-start}")

def generate_response(instruction: str, **kwargs) -> str:
    start = timeit.default_timer()
    generation_pipeline = InstructionTextGenerationPipeline(model=model, tokenizer=tokenizer, **kwargs)
    res = generation_pipeline(instruction)[0]["generated_text"]
    end = timeit.default_timer()
